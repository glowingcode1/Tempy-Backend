...
const POWER_LIFTING_FEDERATIONS_CACHE_KEY = "powerLiftingFederations";
...
const createFederation = async ({ title, status }) => {
  ...
  await invalidate(POWER_LIFTING_FEDERATIONS_CACHE_KEY);
  return saved;
};

const getFederations = async (filter = {}) => {
  return cache({
    namespace: POWER_LIFTING_FEDERATIONS_CACHE_KEY,
    params: filter,
